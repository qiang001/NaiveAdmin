...
module.exports = {
  ...
  createUser: async (payload) => {
    ...
    payload.password = CryptoJS.enc.Base64.stringify(
      CryptoJS.HmacSHA256(payload.password, "sCCMS-user")
    );
  ...
  updateUser: async (id, payload) => {
    let ifExistName = await User.findOne({
      name: payload.name,
      _id: { $ne: id },
    });
    if (ifExistName) {
    ...
    let ifExistUsername = await User.findOne({
  ...
  resetPassword: async (id, password) => {
    let hash = CryptoJS.enc.Base64.stringify(
      CryptoJS.HmacSHA256(password, "sCCMS-user")
    );
